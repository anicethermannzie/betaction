...
from src.models.prediction import EntitlementMixin
...
class OddsAnomalyResponse(EntitlementMixin):
    success: bool = True
    fixture_id: int
...
class TopVsBottomResponse(EntitlementMixin):
    success: bool = True
    fixture_id: int
...
class DeepAnalysisPayload(_Loose):
    ...
    # home_form/away_form/h2h/standings/top_vs_bottom/smart_filter are withheld
    # (emptied to {}) for free callers — see services/entitlements.py.
    # limit_deep_analysis. "How We Got Here" (the 5-factor breakdown) is a VIP
    # row on the pricing page; the same rule applies to this equivalent
...
class DeepAnalysisResponse(EntitlementMixin):
    success: bool = True
    fixture_id: int
...
class SmartPicksListResponse(EntitlementMixin):
    success: bool = True
    count: int
    date: str
