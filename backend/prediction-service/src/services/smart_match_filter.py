"""
Smart match auto-selection filter.
...
class SmartMatchFilter:
    ...
    def filter_interesting_matches(self, matches: list[dict]) -> list[dict]:
        ...
        for match in matches or []:
            ...
            except Exception:  # never let one bad match break the batch
                logger.exception(
                    "Skipped a match while filtering smart picks",
                    extra={"fixture_id": match.get("fixture_id")},
                )
                continue
