"""
Test fixtures for the deep-analysis feature — NOT production data.

These four match_data dicts used to live in src/services/deep_analysis_mock.py
and were served to real clients as a fallback whenever match-service was
unavailable or a request happened to hit one of these four fixture ids. That
was removed during the production readiness audit (see
docs/PRODUCTION_READINESS_AUDIT.md and deep_analysis_service.py's module
docstring) — the exact same fabrication pattern found in the old ticket
generator's mock fixtures.

They remain useful here: analyzer unit tests need realistic, hand-tuned inputs
that exercise specific branches of the decision trees (a clean top-vs-bottom
case, an odds anomaly, a low-scoring bottom side, a case with no market in
range). That is a legitimate use of fixture data — it never leaves the test
suite.
...
ALL_FIXTURES: list[dict] = [MATCH_1, MATCH_2, MATCH_3, MATCH_4]
