...
resource "aws_s3_bucket" "frontend" {
  bucket = "betaction-frontend-${random_id.bucket_suffix.hex}"

  tags = merge(var.tags, {
    Name = "${var.project}-frontend-bucket-${var.environment}"
  })
}
...
resource "aws_cloudfront_origin_access_control" "main" {
  ...
  description                       = "OAC for BetAction S3 frontend bucket"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  # SPA fallback: 403/404 from S3 → index.html (for React Router)
